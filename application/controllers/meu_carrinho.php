	...
	if(isset($_GET['carrinho']) && isset($_GET['id'])){	
		...
		if($_GET['carrinho'] == 'produto'){
			...
			SELECT 
				...
				TV.ValorProduto
			FROM 
				Tab_Produtos_Pagamento AS TP
					LEFT JOIN Tab_Valor_Pagamento AS TV ON TV.idTab_Produtos_Pagamento = TP.idTab_Produtos_Pagamento
					WHERE 
				TV.idTab_Valor_Pagamento = '".$id_valor."' AND
				TV.AtivoPreco = 'S' AND
				TV.VendaSitePreco = 'S'
			ORDER BY 
			...
			if(mysqli_num_rows($read_produto) > '0'){
				foreach($read_produto as $read_produto_view){
					$id_produto 	= $read_produto_view['idTab_Produtos_Pagamento']; 
		...
		if($_GET['carrinho'] == 'promocao'){
			...
			FROM 
				Tab_Promocao AS TPM
				LEFT JOIN Tab_Valor_Pagamento AS TV ON TV.idTab_Promocao = TPM.idTab_Promocao
			WHERE 
			...
			if(mysqli_num_rows($read_promocao) > '0'){
				foreach($read_promocao as $read_promocao_view){
					$id_valor 	= $read_promocao_view['idTab_Valor_Pagamento']; 
					$read_produto = mysqli_query($conn, "
					SELECT 
						TP.idTab_Produtos_Pagamento,
						TP.Nome_Prod,
