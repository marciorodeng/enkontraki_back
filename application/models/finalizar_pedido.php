	...
	*/
	...
	if(count($_SESSION['carrinho'.$_SESSION['id_Cliente'.$idSis_Empresa]]) == '0'){
		...
		echo "<script>window.location = 'meu_carrinho.php'</script>";
			...
			$insert_pedido = "INSERT INTO App_Pagamento(idTab_Modulo,
														idTab_TipoRD,
														idApp_Cliente,
														idSis_Usuario,
														Associado,
            ...
            if ($id_pedido === FALSE) {
				...
				foreach($_SESSION['carrinho'.$_SESSION['id_Cliente'.$idSis_Empresa]] as $id_produto => $qtd_produto){
					...
					*/
					...
					SELECT 
						TPS.idTab_Produtos_Pagamento AS idProduto,
						TPS.Nome_Prod,
						TPS.NomeProdutos,
						TPS.Arquivo,
						TPS.Comissao,
					...
					WHERE 
						TV.idTab_Valor_Pagamento = '".$id_produto."' 
					ORDER BY 
					...
					FROM 
						Tab_Produtos_Pagamento AS TPS
							...
							LEFT JOIN Tab_Opcao AS TOP1 ON TOP1.idTab_Opcao = TPS.Opcao_Atributo_2
					WHERE 
						TV.idTab_Valor_Pagamento = '".$id_produto."' AND
						TPS.Prod_Serv = 'P'
					ORDER BY 
						TV.idTab_Valor_Pagamento ASC
					");
					...
					$insert_itens_pedido_produto = "INSERT INTO App_Produto_Pagamento(idApp_Cliente,
																					idSis_Usuario,
																					idSis_Empresa,
																					idApp_Pagamento,
																					idTab_Produto_Pagamento,
																					idTab_Valor_Produto,
				...
				$insert_parcela = "INSERT INTO App_Parcelas_Pagamento(idTab_Modulo,
																	idTab_TipoRD,
																	Quitado,
																	DataVencimento,
																	ValorParcela,
